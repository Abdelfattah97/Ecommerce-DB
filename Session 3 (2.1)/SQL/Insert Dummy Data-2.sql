...
DO $$
BEGIN
    Raise Notice 'Customers Inserted';
END $$;
...
DO $$
BEGIN
    Raise Notice 'Products Inserted';
END $$;
...
SELECT 
-- random date between '2024-06-01' and '2024-12-31'
'2024-06-01'::date + (random() * ( '2024-12-31'::date - '2024-06-01'::date ))::int AS order_date,
0, 
-- random customer_id from 1 to 10K
(floor(random() * 10000) + 1)::int AS customer_fk
...
SELECT
    (floor(random() * 100000) + 1) as order_fk,
    (floor(random() * 5) + 1) as quantity,
    p.price,
    (floor(random() * 65) + 1)::int as product_fk
FROM
    generate_series(1, 1000000) AS gs(id)  -- Generates 1,000,000 rows
JOIN 
    product AS p ON (floor(random() * 65) + 1)::int = p.product_id;

-- select * from "order_item";

DO $$
BEGIN
    Raise Notice 'Order Items Inserted';
END $$;
...
from(
select order_fk, sum(quantity*unit_price) as total_amount 
from order_item 
group by order_fk) as sub
where  o.order_id = sub.order_fk ;

DO $$
BEGIN
    Raise Notice 'Total Amount Updated';
END $$;
